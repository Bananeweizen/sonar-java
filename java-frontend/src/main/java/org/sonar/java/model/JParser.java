...
public class JParser {
  ...
  private Deque<LabelBindingCouple> labels;
  ...
  private void usageLabel(@Nullable IdentifierTreeImpl node) {
    ...
    labels.stream()
      ...
      .map(couple -> couple.binding)
      .findFirst()
      .ifPresent(binding -> {
        binding.usages.add(node);
        node.labelBinding = binding;
      });
  }
  ...
  private StatementTree convertStatement(@Nullable Statement node) {
    ...
    switch (node.getNodeType()) {
      ...
      case ASTNode.LABELED_STATEMENT: {
        ...
        JLabelSymbol binding = new JLabelSymbol(label.name());
        labels.push(new LabelBindingCouple(label.name(), binding));
  ...
  private static final class LabelBindingCouple {
    final String label;
    final JLabelSymbol binding;

    public LabelBindingCouple(String label, JLabelSymbol binding) {
      this.label = label;
